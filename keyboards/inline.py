...
def meals_keyboard(day: WeekDay, kilo_type: KiloType):
    keyboard = [
        [InlineKeyboardButton(text=meal.value, callback_data=MealCallback(day=day.name, meal=meal.name, action="show", kilo_type=str(kilo_type.value)).pack())]
...
def meal_nav_keyboard(day: WeekDay, meal: MealType, variant_index: int, kilo_type: KiloType):
    buttons = [
        InlineKeyboardButton(text="⬅️ До вибору прийому", callback_data=DayCallback(day=day.name, kilo_type=str(kilo_type.value)).pack())
    ]
    
    meal_obj = newWeekMenu.days[day].meals[meal]
    if len(meal_obj.answers) > 1:
        next_variant_index = (variant_index + 1) % len(meal_obj.answers)
        buttons.append(InlineKeyboardButton(text="Інший варіант", callback_data=MealCallback(day=day.name, meal=meal.name, action="show", variant_index=next_variant_index, kilo_type=str(kilo_type.value)).pack()))

    meal_types = list(MealType)
    current_meal_index = meal_types.index(meal)
    
    if current_meal_index < len(meal_types) - 1:
        buttons.append(InlineKeyboardButton(text="➡️ Далі", callback_data=MealCallback(day=day.name, meal=meal.name, action="next", kilo_type=str(kilo_type.value)).pack()))
    else:
        buttons.append(InlineKeyboardButton(text="✅ Завершити", callback_data=DayCallback(day=day.name, kilo_type=str(kilo_type.value)).pack()))
